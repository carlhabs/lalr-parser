...
class Etat0; class Etat1; class Etat2; class Etat3; class Etat4;
class Etat5; class Etat6; class Etat7; class Etat8; class Etat9;

class Etat0 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat1 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat2 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat3 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat4 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat5 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat6 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat7 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat8 : public Etat { public: bool transition(Automate& a, Symbole* s); };
class Etat9 : public Etat { public: bool transition(Automate& a, Symbole* s); };
...
Automate::~Automate() {
   for (std::size_t i = 0; i < pileSymboles.size(); i++) delete pileSymboles[i];
   for (std::size_t i = 0; i < pileEtats.size(); i++) delete pileEtats[i];
}
...
bool Automate::analyser() {
   ...
   while (!accepte) {
      ...
      if (!e->transition(*this, s)) return false;
   }
   return true;
}

int Automate::resultat() const {
   if (pileSymboles.empty()) return 0;
...
bool Etat1::transition(Automate& a, Symbole* s) {
   if (*s == PLUS) { a.decalage(s, new Etat4()); return true; }
   if (*s == MULT) { a.decalage(s, new Etat5()); return true; }
   if (*s == FIN)  { delete s; a.accepter(); return true; }
   a.erreur("etat1"); return false;
}
...
bool Etat6::transition(Automate& a, Symbole* s) {
   if (*s == PLUS)      { a.decalage(s, new Etat4()); return true; }
   if (*s == MULT)      { a.decalage(s, new Etat5()); return true; }
   if (*s == CLOSEPAR)  { a.decalage(s, new Etat9()); return true; }
   a.erreur("etat6"); return false;
}

bool Etat7::transition(Automate& a, Symbole* s) {
   if (*s == MULT) { a.decalage(s, new Etat5()); return true; } // priorité *
   if (*s == PLUS || *s == CLOSEPAR || *s == FIN) {
      a.reduire(2); // assoc gauche +
      return true;
   }
   a.erreur("etat7"); return false;
}
